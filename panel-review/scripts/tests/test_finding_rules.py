...
FIXTURE_RUN_DIR = os.environ.get("PANEL_REVIEW_REPLAY_DIR", os.path.join(
...
class HarnessLegIngestTest(unittest.TestCase):
    ...
    def render(self, item, reviewer_id="adversarial-claude"):
        root = tempfile.mkdtemp(prefix="panel-review-harness-")
...
class GapClusterTest(unittest.TestCase):
    ...
    def run_dir_with(self, item):
        root = tempfile.mkdtemp(prefix="panel-review-tags-")
        self.addCleanup(shutil.rmtree, root, True)
