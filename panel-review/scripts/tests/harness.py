...
class Workspace(object):
    ...
    def __init__(self, models=None, tiers=None, effort=None, billing="free",
        ...
        self.root = tempfile.mkdtemp(prefix="panel-review-test-")
        ...
        self.connector = self.override("connectors/{0}.json".format(CONNECTOR), {
            ...
            "api_key_secret": None,
            "api_key_env": "PANEL_REVIEW_TEST_KEY",
    ...
    def env(self, extra=None):
        ...
        environment["PANEL_REVIEW_TEST_KEY"] = "not-a-real-key"
        # **Whether a harness is present is a property of the machine, and no test may read the
        # real one.** `judge_lib.harness_present()` asks whether `panel-judge` is installed in
    ...
    def apply_env(self):
        ...
        keys = ("FAKE_BACKEND_", "PANEL_REVIEW_TEST_KEY", judge_lib.HARNESS_AGENTS_DIR_ENV,
                paths_lib.USER_ROOT_ENV)
        os.environ.update({k: v for k, v in self.env().items() if k.startswith(keys)})

    def user_root(self):
        """This workspace's stand-in for `~/.config/panel-review`. Not created until used."""
    ...
    def override(self, relpath, data=None, text=None):
        """Write a file into `<root>/.config/panel-review/`, the cascade's first root."""
...
def shipped(workspace=None, user=None):
    ...
    paths = paths_lib.Paths(
        workspace=workspace or os.path.join(tempfile.gettempdir(), "panel-review-no-such-workspace"),
        user=user or os.path.join(tempfile.gettempdir(), "panel-review-no-such-user"))
