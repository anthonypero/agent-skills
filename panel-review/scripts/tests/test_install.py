...
the catalogue endpoint as a `--url`, and `install.sh` passes `$PANEL_REVIEW_CATALOGUE_URL`
...
class InstallTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.root = tempfile.mkdtemp(prefix="panel-review-install-")
        self.addCleanup(shutil.rmtree, self.root, True)
        self.workspace_root = os.path.join(self.root, ".config", "panel-review")
    ...
    def install(self, args=None, env=None, path_prefix=None):
        environment = {
            ...
            "PANEL_REVIEW_CATALOGUE_URL": "file://" + self.catalogue,
            # No test reads the developer's own ~/.config; this one does not exist.
            "PANEL_REVIEW_USER_DIR": os.path.join(self.root, "no-user-tier"),
        }
...
class HappyPathTest(InstallTestCase):
    ...
    def test_it_reports_the_user_tier_without_creating_one(self):
        ...
        result = self.install(env={"PANEL_REVIEW_USER_DIR": user_root})
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_check_only_reports_a_user_tier_that_does_exist(self):
        ...
        result = self.install(args=["--check-only"],
                              env={"PANEL_REVIEW_USER_DIR": user_root,
                                   "PANEL_REVIEW_CATALOGUE_URL": ""})
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(user_root, result.stdout)
    ...
    def test_check_only_makes_no_catalogue_call_at_all(self):
        before = _read(self.registry)
        result = self.install(args=["--check-only"], env={"PANEL_REVIEW_CATALOGUE_URL": ""})
        self.assertEqual(result.returncode, 0, result.stderr)
...
class HarnessJudgeInstallTest(InstallTestCase):
    ...
    `$PANEL_REVIEW_HARNESS_AGENTS_DIR` points at a temp directory here, so nothing is written
    ...
    def setUp(self):
        ...
        self.judge = os.path.join(self.agents_dir, "panel-judge.md")

    def install(self, args=None, env=None, path_prefix=None):
        environment = {"PANEL_REVIEW_HARNESS_AGENTS_DIR": self.agents_dir}
        environment.update(env or {})
...
class FailureTest(InstallTestCase):
    ...
    def test_no_python_at_all_exits_non_zero_with_a_readable_message(self):
        result = self.install(env={"PATH": os.path.join(self.root, "empty"),
                                   "PANEL_REVIEW_PYTHON": "python-that-is-not-installed"})
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Python 3.10", result.stderr)

    def test_a_catalogue_that_does_not_answer_leaves_the_registry_untouched(self):
        before = _read(self.registry)
        result = self.install(env={"PANEL_REVIEW_CATALOGUE_URL": "file://" + os.path.join(self.root, "nope.json")})
        self.assertNotEqual(result.returncode, 0)
