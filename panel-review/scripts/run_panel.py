...
`<workspace>/.config/panel-review/` first, then `~/.config/panel-review/`, then the skill
...
judge wherever `install.sh` has put `panel-judge` in the harness agents directory — runs
...
def parse_args(argv):
    ...
    parser.add_argument("--workspace", default=None,
                        help="The project holding the artifact. Every file resolves from <workspace>/.config/panel-review/ first, ~/.config/panel-review/ second and the skill package third; config.json and the model files deep-merge rather than replacing (default: the working directory)")
