...
subscription, a harness leg. `zai-coding.json` is that claim made good — the owner's z.ai GLM
coding-plan subscription, a second instance of the *same* driver with a different base URL, key and
billing posture, and no line of driver code that knows it exists. Selecting it is one key,
`default_connector`, because the tiers map is derived per connector (see `registry.derive_tiers`):
that endpoint's model files are the only ones in its map, so nothing about the default map moves.
It is a whole file through the cascade — first hit wins, no merge —
...
def refusal(connector, seats, projection_usd=None, flag="--approve-spend"):
    ...
    lines.append(
        ...
        "the connector file, under ~/.config/panel-review/connectors/ or the project's "
        ".config/panel-review/connectors/.")
    return "\n".join(lines)
