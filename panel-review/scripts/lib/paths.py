...
**The skill package is read-only.** Nothing in a run ever writes into `skills/panel-review/`, and
every file a run loads is resolved through three roots, in this order:

1. `<workspace>/.config/panel-review/` — the project holding the artifact under review, named by
...
2. `~/.config/panel-review/` — **the user tier**, this machine's owner. `$XDG_CONFIG_HOME` is
   honoured when it is set, so the root is `$XDG_CONFIG_HOME/panel-review/` there. It exists so a
...
`.config/panel-review/backends/azure_openai.py` — and no full layout, so this module defines one:
...
WORKSPACE_SUBDIR = os.path.join(".config", "panel-review")

# The user tier's directory name, under `$XDG_CONFIG_HOME` or `~/.config`.
USER_SUBDIR = "panel-review"
...
# `$PANEL_REVIEW_HARNESS_AGENTS_DIR` keeps the harness-judge check off the real agents directory.
USER_ROOT_ENV = "PANEL_REVIEW_USER_DIR"
...
def user_root(environ=None):
    """The user tier's root: `$XDG_CONFIG_HOME/panel-review`, else `~/.config/panel-review`.
