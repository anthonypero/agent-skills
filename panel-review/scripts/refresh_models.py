...
immutable puts a model file under `<project>/.config/panel-review/models/` or
`~/.config/panel-review/models/` first, after which nothing writes into the package.
...
def fetch_catalogue(url=CATALOGUE_URL, opener=None):
    ...
    headers = {"Accept": "application/json", "User-Agent": "panel-review/refresh_models"}
...
def parse_args(argv):
    ...
    parser.add_argument("--workspace", default=None, help="The project whose registry to refresh; its .config/panel-review/models/ wins over this machine's and over the packaged one (default: the working directory)")
