...
Framework §9 driver shape, minus the tool arguments (panel-review personas are tool-less):
...
def dispatch_detailed(system_prompt, user_prompt, model, backend_entry, json_schema, http_request_fn):
    ...
    headers = {
        ...
        "X-Title": backend_entry.get("x_title", "panel-review"),
    }
    ...
    if not (text or "").strip():
        # Some providers return an empty `content` and put everything in the reasoning field —
        # typically when the completion cap was spent on reasoning tokens. Fall back rather than
        # fail on nothing.
        #
        # **Two spellings, because vendors disagree about the name.** OpenRouter returns `reasoning`;
        # z.ai's coding-plan endpoint returns `reasoning_content` and never populates `reasoning`, so
        # a fallback that knew only the first name would raise "no content" on a response that
        # carried the whole answer. Both are read here, in that order, and the note says which one
        # answered — this is a field-name difference in the OpenAI-compatible wire format, not a
        # vendor special case, and nothing below branches on the endpoint.
        text, used = "", None
        for field in ("reasoning", "reasoning_content"):
            ...
            if fallback:
                text, used = fallback, field
                break
        if used:
            notes.append("content was empty; fell back to the `{0}` field".format(used))
