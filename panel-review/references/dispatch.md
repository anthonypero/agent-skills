...
Reachable two ways, and they produce the same seat state. **`--draft`** composes the whole run against the `harness` connector, so every seat lands here and `run_panel.py` prints the spawn block itself; that is the draft pass, and it is the ordinary route now. **`--skip-claude`** is the narrower one it always was: a mixed run where the explicitly seated `claude` seats are left to the session and the rest go through OpenRouter. Either way the leg exists for the same reason — a Claude opinion on the subscription rather than on the API. Its constraints, stated plainly:

- **Model is pinned at spawn.** The host passes an explicit model override — `opus` on a draft run, which the spawn block states — never `subagent_type: "fork"`, which inherits the parent's model, and never Fable: the rule binds every reviewer seat on this leg, and the harness judge is the one written-in exception because it is not a seat. On a draft run the manifest records the model the seat resolved to, because the spawn block is what names it; under `--skip-claude` the seat resolved against the metered map and the subagent will not run that model, so it stays null there.
...
- **Writes are staged.** The seat writes into a seat-private staging directory **outside** the run directory — `<run-dir>/../.panel-staging/<run-id>/<reviewer-id>/`, which `lib/runs.py`'s `staging_dir()` computes — and `render_harness_report.py` validates it by the same code path the OpenRouter leg is held to. No seat is given a path into a directory holding a sibling's report. **The move into the run directory is the operator's `mv`**, not the script's: nothing in the package moves a staged report, and `SKILL.md` step 5 says so.
- **Blinding is prompt-enforced, not sandboxed**, as above.

Each brief must carry the persona body verbatim plus the finding schema, the artifact and references by path with the instruction to read only those, the instruction to write its report to its staging path and nothing else, the instruction to return a digest under 2000 characters, and the instruction not to look for or ask about any other reviewer's output. On a draft run `lib/drafts.py` composes exactly that block, one per seat, and it also names the **envelope fields no script fills in** — `artifact` above all, which `render_harness_report.py` cannot know and whose absence fails validation after the turn has been spent.

## The judge leg

One harness agent, and it is not a reviewer. `agents/judge.md` installs as `panel-judge`, pinned to frontier Claude at `high` effort with three read tools and one `Write` bound to its staging path, and it supplies the **judgment patch** on an unattended run where it is installed. It is the same job the `synthesis` persona does under the same rubric — its body references `agents/synthesis.md` rather than restating it — and the differences are all about tools: the persona is handed everything in one message and this goes and reads it, from the package, the run directory and the pinned `inputs/` copies, and nothing else.

It follows the harness leg's rules, not the OpenRouter leg's. Its model is pinned in the agent definition rather than at spawn. It writes to a seat-private staging path, `<run-dir>/../.panel-staging/<run-id>/panel-judge/judgment.json`, and never into the run directory. Its blinding is prompt-enforced. It costs the run nothing, so `manifest.judge` records it with `leg: harness` and a null cost, and `cost_usd_total` does not move.
...
Every seat goes through a **connector**: a driver module loaded by its config entry's `type`. One driver, `openai_compat`, serves every family — Anthropic included — through one OpenAI-compatible endpoint, and **two connector files instantiate it**. `openrouter.json` is the metered default. `zai-coding.json` is the owner's z.ai GLM coding-plan subscription at `https://api.z.ai/api/coding/paas/v4` — the `/coding/` path is what bills the plan, and z.ai's general API refuses the same token — keyed from `global/ZAI_CODING_API_KEY`, `billing: subscription`, no catalogue, serving the one model `glm-5.3`. **The driver does not know which of the two it is talking to**: no branch in `openai_compat.py` names an endpoint. The one accommodation it makes is a field-name difference in the wire format — when `content` comes back empty, the fallback reads `reasoning` and then `reasoning_content`, because OpenRouter uses the first name and z.ai the second, and a fallback that knew one name would report "no content" on a response that carried the whole answer.

A project chooses between them with one key, `"default_connector"`, in a `config.json` fragment under `<project>/.config/panel-review/`. The tier map is derived per connector, so that key is the whole choice: the OpenRouter map is untouched, and the `zai-coding` map is the single cell `frontier/glm`, so `--tier standard` or `--tier fast` on it refuses cleanly. A run on it is therefore single-family, and a multi-family template collapses onto `glm` through the re-seat rule rather than refusing.

A `type` that names a `.py` file, or that resolves to one under `<workspace>/.config/panel-review/backends/`, is loaded from that path. That is how a project binds a confidential review to a connector carrying a data agreement without editing the read-only package. `backends/base.py` states the contract a driver must satisfy; `scripts/backends/__init__.py` loads it.

The key is resolved by `dispatch.py` itself, from **the connector file's own** `api_key_secret` and `api_key_env` — for the default that is `skills/lastpass/scripts/lp get global/OPENROUTER_API_KEY` first, then `$OPENROUTER_API_KEY` in the environment, then exit naming both paths it tried; for `zai-coding` it is `global/ZAI_CODING_API_KEY` and `$ZAI_CODING_API_KEY` by the same chain. A second endpoint therefore brings its own credential and nothing central has to learn about it. A connector whose `api_key_secret` is null skips the vault leg. **No key value is written to any file in this repo**, which is public.
