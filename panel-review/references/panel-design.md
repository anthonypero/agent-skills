...
A template is a starting point, not a wall: compose seats ad hoc, or drop an edited copy under `<workspace>/.config/panel-review/panels/` and it replaces the shipped one whole.
...
**A second gate sits in front of the budget one and answers a different question.** The shipped `openrouter` connector is `billing: metered`, so every call costs money on an account and no seat bound to it is dispatched without `--approve-spend`. `--approve-budget` says "this projection is more than the limit I set and I accept it"; `--approve-spend` says "you may bill this endpoint at all". A run can be comfortably under budget and still be the first time anybody said yes to paying. A machine that has decided to trust an endpoint sets `requires_approval: false` in its own copy of the connector file, under `~/.config/panel-review/connectors/`, and the manifest records both that the gate was off and which root turned it off. The panel is never silently shrunk to fit: that changes the evidentiary value of the review without telling anyone.

**A subscription endpoint is priced at $0 and is never gated, and one now dispatches.** `zai-coding` is the owner's z.ai GLM coding plan: `billing: subscription`, so its model file carries explicit zero prices — a fact about the plan rather than a missing number — every seat on it projects at $0.0000, and the spend gate records `required: false` without asking. A project selects it with one key, `{"default_connector": "zai-coding"}` in `<project>/.config/panel-review/config.json`, because the tier map is derived per connector and that endpoint's map is the single cell `frontier/glm`, and only that tier: `--tier standard` or `--tier fast` on it refuses. **What that costs in evidence is the thing to weigh, not what it costs in money.** One cell means one family, and a multi-family template run against it does not refuse — every seat is re-seated onto `glm` and the run proceeds with a `min_families` shortfall in the method caveat, while the seat ids still read `fidelity-openai` and `consistency-kimi`. Four lenses on one model is a draft pass, with the same standing `draft-review` has: read its findings one by one, and do not read its agreement counts as corroboration.
