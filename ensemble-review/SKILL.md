...
`--workspace <project>` seeds that project's own `models.json` instead of the package's, `--dry-run` shows what the refresh would change without writing, and `--check-only` does the two checks, **reports whether the judge is installed without installing it**, and makes no catalogue call at all. Any failure exits non-zero with the reason and leaves the registry untouched.
...
| --- | --- |
| the template | `name`, `description`, `requires_references`, `min_families`, `tier`, `reconciler`, `auto_apply`, `verify_web`, `seats`, `optional_seats`, `synthesis`, `deferred`, `routes_to` |
| a seat, in `seats` or `optional_seats` | `lens`, `family`, `tier`, `suffix`, `note` |
| the `synthesis` block | `family`, `tier` |
