...
3. **A report that does not validate** gets **one repair re-ask**, which is the only prompt that does quote the previous response back, because there the previous response is the thing to fix. A second failure retires the seat: the raw response is kept at `<reviewer-id>.invalid.txt`, the attempt record at `<reviewer-id>.failed.json`, and the seat is recorded as **missing** in the manifest and in the reconciliation. It is never silently dropped.
