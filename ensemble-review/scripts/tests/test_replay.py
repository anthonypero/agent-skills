...
DEFAULT_ARTIFACT = os.path.join(
    os.path.dirname(os.path.dirname(SKILL_DIR)),
    ".agents", "subprojects", "ensemble-review", "design", "v1-spec.md",
)
ARTIFACT = os.environ.get("ENSEMBLE_REVIEW_REPLAY_ARTIFACT", DEFAULT_ARTIFACT)
...
def _require_fixtures():
    ...
    """
    for path, what in (
        (RUN_DIR, "the run 1 fixture directory (override with ENSEMBLE_REVIEW_REPLAY_DIR)"),
        (os.path.join(RUN_DIR, "judgment.json"), "the judgment patch fixture"),
        (os.path.join(RUN_DIR, "manifest.json"), "the run manifest"),
        (ARTIFACT, "the pinned artifact (override with ENSEMBLE_REVIEW_REPLAY_ARTIFACT)"),
    ):
