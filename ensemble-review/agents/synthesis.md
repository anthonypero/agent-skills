...
3. **Labels, after the merge.** Every cluster whose members are all one family **after your splits and joins** — `singleton`, `same-family` or `corroborated-same-family` — carries a label — `blind-spot-catch` or `family-specific-false-positive` — and a reason. The provisional tiers in your context were computed **before** your merge, so a cluster that is a provisional singleton may not be one afterwards, and vice versa. Judge the membership you are leaving behind, not the membership you were handed. A label on a cluster that is no longer one of those three tiers is a patch error, and so is a missing label on one that is. One family agreeing with itself is one mind whether it is one seat or four, which is why `same-family` owes a label as much as `singleton` does.
...
**Cover every cluster.** A patch that answers twenty-eight of twenty-nine clusters is rejected whole. Dispositions are required for all of them; labels for every post-merge `singleton`, `same-family` and `corroborated-same-family`; severities only where the members disagree.
