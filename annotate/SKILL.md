...
Before the first review in a workspace, check whether the stack is up — this is cheap and idempotent:

```sh
test -f "$HOME/.annotate/runtime.json" || sh install.sh
```

`install.sh` (equivalently `annotate setup`) brings up the whole stack in one command: the data dir, the generated `runtime.json`, a provisioned Chrome-for-Testing (or reused system Chromium), the dedicated browser profile, the lazy-singleton server, and the loaded extension (heartbeat-verified). Re-running on a set-up workspace is a no-op. If setup degrades, it prints a short, near-mechanical checklist for the one or two steps a given browser/OS refused — relay those to the human.
...
- **Session** — reuse the harness session ID if your environment exposes one; otherwise the script generates a stable `<timestamp>-<8char>`. To keep all of a run's reviews together, pass the same `--session <id>` on every call in that run; the artifact tier already separates multiple files within one session. Use a *fresh* `--session <id>` only to isolate a concurrent, independent review.
...
1. **Read `feedback`.** Each entry is one anchor: `type: "comment"` (intent — you decide the fix) or `type: "edit"` (a literal `original` → `replacement` the human wrote, which you apply and whose consequences you surface). The `anchor` names the exact region — a `source` line/keyPath/cell, a `text` quote, or a `spatial` point/box.
